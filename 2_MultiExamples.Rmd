...
The depth layers values are stored in the `names` of the brick object.
```{r, comment =""}
...
depth
```
The depth layers goes from 2m to 700m depth, with an increasing interval between depth layer (from a 4m interval near the surface to a 40m interval at the bottom).

#### Load the hauls shapefile
We will extract the temperature profile of July 2015 for the hauls positions of BITS carried out in the first quarter of 2016. So let's load the shapefile *Hauls_BITSTVLQ1_2016.shp*.
...
#### Visualize
One of the key step before extracting values is to visualize the two datasets. The function `plot()` can create a map of a `brick` object, with the added parameters `y` telling which layer to be plotted. So let's  overlay the surface temperature (`y=1`) and the hauls. 
```{r, comment ="", eval=FALSE, fig.path='Figures/', fig.height=7, fig.width=7}
...
```{r, echo = FALSE, fig.path='Figures/', fig.height=5, fig.width=8}
par(mfrow=c(1,2))
...
plot(temp072015, y=13, main="Temperature at 50m depth", 
     col= brewer.pal(9,"YlOrRd"))
title("Temperature at 50m depth", cex=1, font=2)
plot(hauls, add=TRUE, cex=0.5)
map("worldHires", col="grey90", border="grey50", 
    fill=TRUE, add=TRUE)
box()
...
```{r, comment ="", fig.path='Figures/', fig.height=5, fig.width=5}
...
```{r, echo = FALSE, fig.path='Figures/', fig.height=4, fig.width=7}
...
The value extracted does not only correspong to bottom and surface temperature but we can visualize the temperature-depth profile for each point. Because there are 158 points (i.e. hauls), we will use a `for` loop to draw a line for each point

```{r, comment ="", fig.path='Figures/', fig.height=6, fig.width=6}
...
#Create a map
plot(hauls, col=col_dp10$col, pch=16,
...
```{r, results='hide', fig.path='Figures/', fig.height=7, fig.width=8}
...
We can plot the average seasonal pattern of the ICES rectangle. The function `layout` is a flexible framework that allow us to make multiple plot, matching the spatial ditribution of the ICES rectangle. 
...
```{r, echo = FALSE, fig.path='Figures/', fig.height=6, fig.width=7}
